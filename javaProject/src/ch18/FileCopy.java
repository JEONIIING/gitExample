...
public class FileCopy {
	...
	public static void main(String[] args) {
		//복사본 <== (OutputStream객체 사용) [프로그램] <==(InputStram객체 사용) 원본 
		String str="";
		...
		try {
			reader = new BufferedReader(new FileReader(new File(file1))); //한줄짜리 기법
			...
			try {
				if(reader != null) reader.close(); //참조값을 가지고 있으면 리소스를 개별적으로 닫아주는게 좋다. 
				//이렇게 해줘야 메모리 정리가 빨리 됨
			} catch (Exception e2) {
				e2.printStackTrace();
			}
			try {
